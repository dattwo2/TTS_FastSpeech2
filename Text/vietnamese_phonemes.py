...
def parse_word(word, replaced=True, tach_phu_am=False):
    ...
    if len_word>0:
        #xac dinh phu am dau
        if len_word>3:
            if word[:3] in phu_am_dau:
                result.append(word[:3])
                word = word[3:]
            elif word[:2] in phu_am_dau:
                result.append(word[:2])
                word = word[2:]
            elif word[0] in phu_am_dau:
                result.append(word[0])
                word = word[1:]
        elif len_word > 2:
            if word[:2] in phu_am_dau:
                result.append(word[:2])
                word = word[2:]
            elif word[0] in phu_am_dau:
                result.append(word[0])
            ...
            if word[0] in phu_am_dau:
                result.append(word[0])
                word = word[1:]

        len_word = len(word)
        if replaced and len(result) > 0:
            result[0] = phu_am_dau[result[0]]['rep']
